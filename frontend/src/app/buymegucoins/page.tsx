        ...
        {mockResult && (
          <div className="mt-10 bg-white/70 rounded-2xl p-4 border border-orange-100">
            <p className="text-xs text-amber-600 mb-3">
              ※ Stripe未設定のためモック決済情報を表示しています
            </p>
            <dl className="text-sm text-gray-700 space-y-1">
              <div className="flex justify-between"><dt>決済ID</dt><dd>{mockResult.paymentId}</dd></div>
              <div className="flex justify-between"><dt>コイン</dt><dd>{mockResult.coinAmount.toLocaleString()}コイン</dd></div>
              <div className="flex justify-between"><dt>金額</dt><dd>¥{mockResult.amountInYen.toLocaleString()}</dd></div>
            </dl>
          </div>
        )}
      </main>
