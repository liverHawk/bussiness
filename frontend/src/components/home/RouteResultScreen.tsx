...
function buildFromApi(api: RouteGenerateResponse): RouteResultData {
  ...
  return {
    departureName: first?.locationName ?? '出発地',
    departureTime: first ? toTime(first.estimatedHour, first.estimatedMinute) : '—',
    destinationNames: api.timeline.slice(1).map((t) => t.locationName),
    finalDestinationName: last?.locationName ?? '目的地',
    arrivalTime: last ? toTime(last.estimatedHour, last.estimatedMinute) : '—',
    distanceKm: api.totalDistance,
    durationLabel: durationH > 0 ? `約${durationH}時間${durationM}分` : `約${durationM}分`,
    startPosition: [first?.latitude ?? 0, first?.longitude ?? 0],
    endPosition: [last?.latitude ?? 0, last?.longitude ?? 0],
    path,
    waypoints: api.timeline.slice(1, -1).map((t, i) => ({
      id: `w${i + 1}`,
      name: t.locationName,
      emoji: EMOJIS[i % EMOJIS.length],
      arrival: toTime(t.estimatedHour, t.estimatedMinute),
      stay: '約30分',
      lat: t.latitude,
      lng: t.longitude,
    })),
    timeline: api.timeline.map((t, i) => ({
      id: `t${i + 1}`,
      icon: i === 0 ? '🔵' : i === api.timeline.length - 1 ? '🔴' : EMOJIS[i % EMOJIS.length],
      title: t.locationName,
      subtitle: `${toTime(t.estimatedHour, t.estimatedMinute)} ${t.actionLabel}`,
    })),
  }
}
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        setData(buildFromApi(api))
      }
    } catch {
      // sessionStorage が壊れていればモックを使用
    }
  }, [])
    ...
    <div className="relative flex h-screen w-full flex-col overflow-hidden bg-[#fffbf7] text-[#2f2419]">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {data.destinationNames.map((name) => (
                <span key={name} className="flex items-center gap-2">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#d3883f" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="flex-none">
                    <polyline points="9 6 15 12 9 18" />
                  </svg>
                  <span className="rounded-full bg-[#d3883f] px-3 py-1.5 text-xs font-semibold text-white">{name}</span>
                </span>
              ))}
            </div>
          )}
            ...
            <div className="rounded-2xl bg-white p-4 shadow-sm">
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <p className="text-sm font-semibold text-[#2f2419]">{data.durationLabel} ・ {data.distanceKm}km</p>
                </div>
              </div>
      ...
      {detailOpen && (
        <div className="absolute inset-x-0 bottom-0 z-10 max-h-[60vh] overflow-y-auto rounded-t-3xl bg-white px-5 pb-24 pt-4 shadow-[0_-4px_16px_rgba(0,0,0,0.08)]">
          ...
          <div className="mb-4 flex items-end justify-between">
            ...
            <p className="text-sm text-gray-500">{data.distanceKm}km ・ {data.durationLabel}</p>
          </div>
          <ul className="space-y-3">
            {data.timeline.map((step) => (
              <li key={step.id} className="flex items-start gap-3">
                <span className="flex h-8 w-8 flex-none items-center justify-center rounded-full bg-[#fdf1e3] text-base">{step.icon}</span>
                <div>
      ...
      <div className="absolute inset-x-0 bottom-0 z-20 flex items-center justify-between gap-3 border-t border-black/5 bg-[#fffbf7] px-4 py-3">
        <button
          ...
          className={`rounded-full px-5 py-3 text-sm font-semibold shadow-sm transition active:scale-[0.98] ${detailOpen ? 'bg-[#d3883f] text-white' : 'border border-[#d3883f] bg-white text-[#d3883f]'}`}
        >
          ルート詳細
        </button>
        <button
          ...
          className={`flex items-center gap-2 rounded-full px-5 py-3 text-sm font-semibold shadow-sm transition active:scale-[0.98] ${saved ? 'bg-[#d3883f] text-white' : 'border border-[#d3883f] bg-white text-[#d3883f]'}`}
        >
          <svg width="16" height="16" viewBox="0 0 24 24" fill={saved ? '#fff' : 'none'} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
