...
export default function Payment() {
  ...
  const pay = async () => {
    ...
    try {
      ...
      setMessage(`¥${total.toLocaleString()} の決済が完了しました！`);
      setCart([]); setUserId(null); setCoupon("");
    } catch { setError("決済に失敗しました"); }
  };
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl shadow-sm p-5">
            ...
            {userId && (
              <p className="mt-2 text-emerald-600 text-sm font-medium">✓ ユーザー認識完了</p>
            )}
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm p-5">
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {products.map((p) => (
                <button
                  key={p.id}
                  className="w-full flex justify-between items-center px-4 py-2.5 bg-gray-50 hover:bg-indigo-50 rounded-lg transition text-sm"
                  onClick={() => addToCart(p)}
                >
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-2xl shadow-sm p-5">
            ...
            <input
              className="w-full border border-gray-300 rounded-lg px-4 py-2 text-sm focus:border-indigo-500 transition mb-4"
              placeholder="例：SUMMER10" value={coupon} onChange={(e) => setCoupon(e.target.value)}
            />
            ...
            <div className="flex gap-3">
              <button
                className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white py-3 rounded-lg transition font-bold"
                onClick={pay}
              >
                決済する
              </button>
              <button
                className="bg-red-500 hover:bg-red-600 text-white px-4 py-3 rounded-lg transition text-sm"
                onClick={reset}
              >
                リセット
              </button>
